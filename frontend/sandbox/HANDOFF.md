...
3. **Schritt 4 — Parallax (implementiert, wartet auf Review)**: Headphones, LISTEN-UP-Fill und LISTEN-UP-Outline driften so, dass sie nie deckungsgleich sind. Layer-Vorzeichen: Headphones `+`, Outline `−` (entgegengesetzt zu HP), Fill `+` aber schwächer. Magnituden in `cqw` (konsistent mit dem restlichen Header-Sizing): HP 1.5/1.0, Outline 1.2/0.8, Fill 0.6/0.4 cqw pro Achse.
   - **CSS**: Je Layer eine separate `translate: var(--dc-px-{layer}-x, 0) var(--dc-px-{layer}-y, 0)` Property. Bewusst die separate `translate` CSS-Property genutzt (nicht `transform`), damit die bestehenden `transform: translate(…)`-Centering-Werte der LISTEN-UP-Layer unangetastet bleiben — Browser komponiert beide.
   - **JS**: Ein eigener `<script>`-Block direkt nach dem Scroll-Morph-Polyfill. Early-Return bei `prefers-reduced-motion: reduce`. rAF-Lerp (Faktor 0.08) zwischen Target und Current für smoothe Bewegung. Schreibt 6 Vars (`--dc-px-{hp,outline,fill}-{x,y}`) als `cqw`-Strings nach `:root`.
     ...
     - **Coarse Pointer / Touch**: `scroll`-Listener mappt die erste ~0.8 viewport-height Scroll-Strecke linear auf `ty = 0..+1`. `tx` bleibt 0 (keine natürliche X-Quelle auf Touch). Selbe Per-Layer-Vorzeichen → drei Layer driften auseinander beim Scrollen in den Hero.
   - **Migration MITNEHMEN**: Den Parallax-`<script>`-Block + die drei `translate:`-CSS-Properties auf `.hero__headphones`, `.listen-up--fill`, `.listen-up--outline`. Magnituden-Objekt `M` im JS ist die einzige Stelle, an der man die Stärken/Vorzeichen tunt.
   - **Verifiziert via** `/tmp/dc-shots/parallax-test.js`: rest = leere Vars, top-left vs bottom-right invertiert sich, reduced-motion = bleibt leer, Touch nach Scroll = drei verschiedene Y-Werte. Visueller Versatz LISTEN-UP-Fill ↔ Outline klar erkennbar im Screenshot.
